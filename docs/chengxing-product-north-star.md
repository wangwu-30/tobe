...
> 详细方案见 [unified-agent-entry.md](./briefs/unified-agent-entry.md)

- 成形要成为用户在 Agent 世界的统一入口：创作文档、做网站、浏览网页、操作网站、自动化重复任务、监控变化。
- 新增 `renderAs = 'browser'` 画布类型，在中央主表面嵌入真实浏览器。
- 浏览器画布 + bridge 注入构成网页增强平台：评论持久化回显、沉浸式翻译、信息提取、高亮标记、简化阅读。
- AI 可以在浏览器画布上操作网页（点击、输入、截图、提取），用户全程共屏协作、随时接管。
- 操作序列可沉淀为 Playbook，支持参数化回放和复用。

### 第三层：浏览即学习，创作即应用

- 浏览器是知识采集端，创作台是知识应用端，Note 是中间层。
- 用户在任意网页上的标注、评论、高亮，自动提取为 `Note(scope=user, kind=knowledge)`。
- AI 在创作时自动从 Context 中找到并引用这些知识，形成采集→沉淀→应用的闭环。
- 用户日常浏览过程本身就在积累 AI 可用的知识库，不需要手动整理。

### 第四层：开放内容生态（远景探索）

> 当前为方向性占位，详细方案待探索。见 [content-ecosystem-vision.md](./briefs/content-ecosystem-vision.md)

- 成形的创作、评论、知识、版本、Playbook 不应该只封闭在本地。
- 长期方向是定义一套开放的内容交换协议和存储分发标准，让成形产出的内容可以在互联网上流通、被引用、被协作。
- 这不是传统的“导出/发布”，而是让内容本身成为可寻址、可评论、可版本化、可溯源的互联网原生对象。
