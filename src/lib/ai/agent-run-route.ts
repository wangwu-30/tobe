...
export async function handleAgentRunRequest(req: NextRequest) {
  try {
    const context = await resolveAgentRunRequestContext(req);
    return context.payload.researchMode === 'deep'
      ? executeDeepResearchPlanRequest(context, req)
      : executeLightChatRunRequest(context, req);
  } catch (error) {
...
export async function handleLightChatRunRequest(req: NextRequest) {
  try {
    const context = await resolveAgentRunRequestContext(req);
    if (context.payload.researchMode === 'deep') {
...
async function resolveAgentRunRequestContext(req: NextRequest) {
